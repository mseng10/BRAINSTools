...
landmarksConstellationDetector::ComputeFinalRefinedACPCAlignedTransform( SImageType::Pointer original_space_image )
{
  ...
  this->m_orig2msp_img_tfm = itk::ComputeRigidTransformFromGeneric( brainsFitHelper->GetCurrentGenericTransform()->GetNthTransform(0).GetPointer() );
  if( this->m_orig2msp_img_tfm.IsNull() )
    ...
    const VersorRigid3DTransformType::OutputPointType acPointInACPCSpace = LandmarkOrigToACPCTransform->TransformPoint(GetNamedPointFromLandmarkList(this->m_orig_lmks_updated,"AC"));
    ...
    //VersorRigid3DTransformType::OutputPointType newacPointInACPCSpace = LandmarkOrigToACPCTransform->TransformPoint(GetNamedPointFromLandmarkList(this->m_orig_lmks_updated,"AC"));
...
void landmarksConstellationDetector::Compute( SImageType::Pointer orig_space_image )
{
  ...
  this->m_msp_lmk_CenterOfHeadMass =
    eyeFixed2msp_lmk_tfm->TransformPoint( this->m_eyeFixed_lmk_CenterOfHeadMass );
  this->m_msp_lmk_CenterOfHeadMass[0] = 0; // Search starts on the estimated MSP
    ...
    std::cout << "\nPerforming morpohmetric search + local search..." << std::endl;
      ...
      const double err_MSP = std::abs( msp_lmk_RP_Candidate[0]
      ...
      if( globalImagedebugLevel > 1 )
        ...
        if( globalImagedebugLevel > 3 )
          ...
          MakeLabelImage( this->m_msp_img, msp_lmk_RP_Candidate, msp_lmk_AC_Candidate,
                          msp_lmk_PC_Candiate, msp_lmk_VN4_Candidate, LabelImageAName );
          }
        }
      ...
      this->m_orig_lmks_updated["CM"] =
        ...
        if( !hasUserForcedRPPoint )
          {
            m_eyeFixed_lmks["RP"] = this->m_orig2eyeFixed_img_tfm->TransformPoint( this->m_orig_lmks_constant.at("RP") );
          }
        if( !hasUserForcedVN4Point )
          {
            m_eyeFixed_lmks["VN4"] = this->m_orig2eyeFixed_img_tfm->TransformPoint( this->m_orig_lmks_constant.at("VN4") );
          }
        if( !hasUserForcedACPoint )
          {
            m_eyeFixed_lmks["AC"] = this->m_orig2eyeFixed_img_tfm->TransformPoint( this->m_orig_lmks_constant.at("AC") );
          }
        if( !hasUserForcedPCPoint )
          {
            m_eyeFixed_lmks["PC"] = this->m_orig2eyeFixed_img_tfm->TransformPoint( this->m_orig_lmks_constant.at("PC") );
          }
      ...
      // Write some debug images
        ...
        if( globalImagedebugLevel > 3 )
          ...
          MakeLabelImage( this->m_eyeFixed_img, msp_lmk_RP_Candidate,
                          msp_lmk_AC_Candidate, msp_lmk_PC_Candiate,
                          msp_lmk_VN4_Candidate, ResampledMaskmageName );

          std::string OrigMaskImageName
            ( this->m_ResultsDir + "/Orig_Mask.nii.gz" );
          MakeLabelImage( this->m_eyeFixed_img, this->m_orig_lmks_updated.at("RP"),
                          this->m_orig_lmks_updated.at("AC"), this->m_orig_lmks_updated.at("PC"),
                          this->m_orig_lmks_updated.at("VN4"), OrigMaskImageName );
          }
        }
      ...
      LandmarksMapType msp_lmks_from_orig_space;    // named points in ACPC-aligned space
        {
        msp_lmks_from_orig_space["AC"] =
          orig2msp_lmk_tfm->TransformPoint( this->m_orig_lmks_updated.at("AC") );
        msp_lmks_from_orig_space["PC"] =
          orig2msp_lmk_tfm->TransformPoint( this->m_orig_lmks_updated.at("PC") );
        msp_lmks_from_orig_space["RP"] =
          orig2msp_lmk_tfm->TransformPoint( this->m_orig_lmks_updated.at("RP") );
        msp_lmks_from_orig_space["VN4"] =
          orig2msp_lmk_tfm->TransformPoint( this->m_orig_lmks_updated.at("VN4") );
        msp_lmks_from_orig_space["CM"] =
          orig2msp_lmk_tfm->TransformPoint( this->m_orig_lmks_updated.at("CM") );
        msp_lmks_from_orig_space["LE"] =
          orig2msp_lmk_tfm->TransformPoint( this->m_orig_lmks_updated.at("LE") );
        msp_lmks_from_orig_space["RE"] =
          orig2msp_lmk_tfm->TransformPoint( this->m_orig_lmks_updated.at("RE") );
        }

      // Get a copy of landmarks on ACPC plane for eliminating accumulative
      // errors of local search process
      LandmarksMapType raw_msp_lmks = msp_lmks_from_orig_space; // reserve this map to avoid the accumulation of local search errors
     ...
     */
        ...
        for( unsigned int ii = 1; ii <= m_LlsMatrices.size(); ++ii )
          ...
          std::cout << "Processing " << iit->first << "..." << std::endl;
            ...
            this->m_orig_lmks_updated[iit->first] = this->m_orig2msp_img_tfm->TransformPoint( msp_lmks_from_orig_space.at(iit->first) );
              {
              local_msp_lmks_algo_found[iit->first] =  eyeFixed2msp_lmk_tfm->TransformPoint(
                orig2eyeFixed_lmk_tfm->TransformPoint( this->m_eyeFixed_lmks.at(iit->first) )
                );  //TODO: Verify this
              }
            ...
            if( 1 )
              ...
              this->m_orig_lmks_updated[iit->first] = this->m_test_orig2msp_img_tfm->TransformPoint( local_msp_lmks_algo_found.at(iit->first) );
                {
