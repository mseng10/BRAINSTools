...
public:
  ...
  void SetForce_orig_lmk_ACPointRAS( std::vector< float > forceACPointRAS)
  {
    this->m_force_orig_lmk_ACPointLPS = RAS2LPS(forceACPointRAS, "AC");
  }

  void SetForce_orig_lmk_PCPointRAS( std::vector< float > forcePCPointRAS)
  {
    this->m_force_orig_lmk_PCPointLPS = RAS2LPS(forcePCPointRAS, "PC");
  }

  void SetForce_with_lmk_VN4PointRAS( std::vector< float > forceVN4PointRAS)
  {
    this->m_force_orig_lmk_VN4PointLPS = RAS2LPS(forceVN4PointRAS, "VN4");
  }

  void SetForce_with_lmk_RPPointRAS( std::vector< float > forceRPPointRAS)
  {
...
private:
  static std::vector< float > & RAS2LPS(std::vector< float > & RASlmk, std::string name)
  {
    if ( ! RASlmk.empty() )
      {
      //If not empty, then only size 3 is allowed, and must negate first two elements.
        if ( RASlmk.size() != 3 )
          {
            std::cerr << "Forced landmark requires 3 dimensions, " << RASlmk.size()
            << " provided for " << name << ". ";
  ...
  static ImagePointType localFindCenterHeadFunc(ImageType::ConstPointer img );
