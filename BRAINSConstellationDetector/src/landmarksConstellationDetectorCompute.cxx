...
void landmarksConstellationDetector::Compute( SImageType::Pointer orig_space_image )
{
  ...
  const SImageType::PointType msp_lmk_CenterOfHeadMass_zeroed_x = temppnt;
...

    {
      ...
      if( mapHasKey(m_orig_lmks_forced, "RP") )
        {
          ...
          msp_lmks_algo_found["RP"] = eyeFixed2msp_lmk_tfm->TransformPoint( orig2eyeFixed_lmk_tfm->TransformPoint( this->m_orig_lmks_forced.at("RP") ) );
        }
      else
        {
          ...
          msp_lmks_algo_found["RP"] =
            FindCandidatePoints( this->m_msp_img, this->m_msp_img, searchRadiusLR,
      ...
      const double err_MSP = std::abs( msp_lmks_algo_found["RP"][0]
                                       - msp_lmk_CenterOfHeadMass_zeroed_x[0] );
      ...
      if( mapHasKey(m_orig_lmks_forced, "VN4") )
        {
          ...
          msp_lmks_algo_found["VN4"] = eyeFixed2msp_lmk_tfm->TransformPoint( orig2eyeFixed_lmk_tfm->TransformPoint( this->m_orig_lmks_forced.at("VN4") ) );
        }
      else
        {
      ...
      if( mapHasKey(m_orig_lmks_forced, "PC") )
        {
          ...
          msp_lmks_algo_found["PC"] = eyeFixed2msp_lmk_tfm->TransformPoint( orig2eyeFixed_lmk_tfm->TransformPoint( this->m_orig_lmks_forced.at("PC") ) );
        }
      else
        {
          ...
          if( globalImagedebugLevel > 3 )
            {
              ...
              MakeLabelImage( this->m_msp_img, msp_lmks_algo_found["RP"], msp_lmks_algo_found["AC"],
                              msp_lmks_algo_found["PC"], msp_lmks_algo_found["VN4"], LabelImageAName );
            }
        }
      // ============================================================================================
      ...
      if( !mapHasKey(m_orig_lmks_forced, "RP") )
        {
          const SImagePointType tempLoc =
            this->m_test_orig2msp_img_tfm->TransformPoint( msp_lmks_algo_found["RP"] );
      ...
      if( !mapHasKey(m_orig_lmks_forced, "VN4") )
        {
          const SImagePointType tempLoc =
            this->m_test_orig2msp_img_tfm->TransformPoint( msp_lmks_algo_found["VN4"] );
      ...
      if( !mapHasKey(m_orig_lmks_forced, "AC") )
        {
          const SImagePointType tempLoc =
            this->m_test_orig2msp_img_tfm->TransformPoint( msp_lmks_algo_found["AC"] );
      ...
      if( !mapHasKey(m_orig_lmks_forced, "PC") )
        {
          const SImagePointType tempLoc =
            this->m_test_orig2msp_img_tfm->TransformPoint( msp_lmks_algo_found["PC"] );
        ...
        if( globalImagedebugLevel > 3 )
          {
            ...
            MakeLabelImage( this->m_eyeFixed_img, msp_lmks_algo_found["RP"],
                            msp_lmks_algo_found["AC"], msp_lmks_algo_found["PC"],
                            msp_lmks_algo_found["VN4"], ResampledMaskmageName );
        ...
        for( unsigned int LlsMat_idx = 1; LlsMat_idx <= m_LlsMatrices.size(); ++LlsMat_idx )
          {
            ...
            std::cout << "Processing iterative update: " << LlsMatrix_name << "..." << std::endl;
            {
              ...
              LandmarksMapType iteratively_updated_msp_lmks = msp_lmks_computed_iteratively_using_orig_lmks; //Initialize with lmks from orig_space
              ...
              msp_lmks_computed_iteratively_using_orig_lmks[LlsMatrix_name] = iteratively_updated_msp_lmks[LlsMatrix_name];

              // Obtain the position of the current landmark in other spaces
              this->m_orig_lmks_updated[LlsMatrix_name] = this->m_orig2msp_img_tfm->TransformPoint( msp_lmks_computed_iteratively_using_orig_lmks.at(LlsMatrix_name) );
              ...
              if( 1 )
                {
                  ...
                  msp_lmks_algo_found[LlsMatrix_name] =
                    FindCandidatePoints( this->m_msp_img, this->m_msp_img, localSearchRadiusLR,
                                         ...
                                         cc_Max, LlsMatrix_name );

                  // Update landmarks in input and ACPC-aligned space
                  this->m_orig_lmks_updated[LlsMatrix_name] = this->m_test_orig2msp_img_tfm->TransformPoint( msp_lmks_algo_found.at(LlsMatrix_name) );
                  {
